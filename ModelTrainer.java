...
public class ModelTrainer {
    ...
    public void logisticRegression(SparkSession spark) {
        System.out.println();
        Dataset<Row> labelFeatureDf = getDataFrame(spark, true, TRAINING_DATASET).cache();
        LogisticRegression logRegression = new LogisticRegression().setMaxIter(100).setRegParam(0.0);

        Pipeline pl1 = new Pipeline();
        pl1.setStages(new PipelineStage[]{logRegression});

        PipelineModel model1 = pl1.fit(labelFeatureDf);


        LogisticRegressionModel logRegModel = (LogisticRegressionModel) (model1.stages()[0]);
